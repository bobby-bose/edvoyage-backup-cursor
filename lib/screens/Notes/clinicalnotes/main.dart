...
class _ClinicalNotesScreenState extends State<ClinicalNotesScreen> {
  ...
  Future<List<Map<String, dynamic>>> fetchClinicalTopics() async {
    try {
      final response = await http.get(
        Uri.parse('${BaseUrl.baseUrl}/api/clinical-notes/'),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
        },
      ...
      print('Clinical Categories API Status: ${response.statusCode}');
      print('Clinical Categories API Body: ${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        if (decoded['status'] == 'success' && decoded['data'] != null) {
          ...
          print('Unexpected API response format: $decoded');
          return [];
        }
      } else {
      ...
      print('Network error while fetching clinical categories: $e');
      return [];
    }
  }
              ...
              child: FutureBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (topics.isEmpty) {
                    return Center(
                      child: Text(
                        'No clinical categories available',
                        style: TextStyle(
                            fontFamily: 'Poppins', fontSize: 16, color: grey3),
                      ),
                    );
                  }
                  ...
                  return ListView.builder(
                    ...
                    itemCount: topics.length,
                    itemBuilder: (context, index) =>
                        _buildClinicalTopicCard(topics[index]),
                  );
                },
              ),
            ),
